...
def load_state(filename='state.csv'):
    ...
    if not os.path.exists(filename):
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            writer.writerow(["seat", "customer_data"])
        seat_choices = {}
    else:
        seat_choices = {}
...
def seatchooser():
    ...
    def update(s, label, root):
        mail1.send_email(ee,ue)  # Placeholder, implement send_email function
        if label:
            label.config(bg="red")
...
def customersystem():
    ...
    emailentry = Entry(Cusloginframe, font=("Times New Roman", 20, 'bold'), bg='white', fg='black')
    emailentry.place(x=30, y=330, width=410)
